 ...
 * true если ответ с сервера положительный
...
/**
 * Преобразовать элементы формы в js-объект
 *
 * @param selector - Селектор формы
 */
function parseFormToObject(selector) {
    var obj = {};
...
/**
 * Преобразовать элементы формы в json-объект
 *
 * @param selector - Селектор формы
 */
...
/**
 * Пытается разрешить CSRF-токен из специальных метатегов JSP-страницы ("_csrf_header" и "_csrf").
 * А они там появятся, если вставить в заголовок страницы тег <sec:csrfMetaTags />
 * из библиотеки <%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags" %>
 *
 * @return js-объект c единственным узлом, где ключ - имя заголовка, а значение - сам токен.
 * Возвращённый элемент готов для вставки в элемент "headers" объекта "settings" метода jQuery.ajax(),
 * что приведёт к выставлению соответствующего заголовка у аякс-запроса.
